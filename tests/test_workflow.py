...
projects that use ``GroupApprovalTask``, the behavior is identical --
the toolset just forwards to ``task.get_actions(page, user)``.
...
def test_workflow_submit_rejects_anonymous(toolset, bind_user, stream_page):
    with pytest.raises(PermissionDenied):
        bind_user(toolset, None).workflow_submit(page_id=stream_page.pk)


@pytest.mark.django_db
def test_workflow_state_rejects_anonymous(toolset, bind_user, stream_page):
    with pytest.raises(PermissionDenied):
        bind_user(toolset, None).workflow_state(page_id=stream_page.pk)


@pytest.mark.django_db
def test_workflow_approve_rejects_anonymous(toolset, bind_user):
    with pytest.raises(PermissionDenied):
        bind_user(toolset, None).workflow_approve(task_state_id=1)


@pytest.mark.django_db
def test_workflow_reject_rejects_anonymous(toolset, bind_user):
    with pytest.raises(PermissionDenied):
        bind_user(toolset, None).workflow_reject(task_state_id=1)


@pytest.mark.django_db
def test_workflow_cancel_rejects_anonymous(toolset, bind_user):
    with pytest.raises(PermissionDenied):
        bind_user(toolset, None).workflow_cancel(workflow_state_id=1)
...
def test_workflow_state_returns_none_when_no_workflow(
    ...
    assert (
        bind_user(toolset, superuser).workflow_state(page_id=stream_page.pk)
        is None
    )
...
def test_workflow_submit_without_assignment_raises(
    ...
    with pytest.raises(ValueError):
        bind_user(toolset, superuser).workflow_submit(page_id=stream_page.pk)
...
def test_workflow_submit_with_unknown_workflow_id_raises(
    ...
    with pytest.raises(ValueError):
        bind_user(toolset, superuser).workflow_submit(
            page_id=stream_page.pk, workflow_id=999_999
        )
...
def test_workflow_submit_starts_a_workflow(
    toolset, bind_user, superuser, stream_page, workflow
):
    """Explicit workflow_id + superuser -> WorkflowState is created."""
    stream_page.save_revision(user=superuser)
    payload = bind_user(toolset, superuser).workflow_submit(
        page_id=stream_page.pk, workflow_id=workflow.pk
    )
...
def test_workflow_state_after_submit_matches(
    ...
    submit_payload = bound.workflow_submit(
        page_id=stream_page.pk, workflow_id=workflow.pk
    )
    state_payload = bound.workflow_state(page_id=stream_page.pk)
...
def test_workflow_cancel_marks_workflow_cancelled(
    ...
    stream_page.save_revision(user=superuser)
    bound = bind_user(toolset, superuser)
    submitted = bound.workflow_submit(
        page_id=stream_page.pk, workflow_id=workflow.pk
    )
    cancelled = bound.workflow_cancel(workflow_state_id=submitted["id"])
...
def test_workflow_approve_advances_task(
    ...
    payload = bound.workflow_approve(task_state_id=task_state_id, comment="lgtm")
...
@pytest.mark.django_db
def test_workflow_approve_on_missing_task_state_raises(
    toolset, bind_user, superuser
):
    with pytest.raises(ValueError):
        bind_user(toolset, superuser).workflow_approve(task_state_id=999_999)


@pytest.mark.django_db
def test_workflow_cancel_on_missing_state_raises(toolset, bind_user, superuser):
    with pytest.raises(ValueError):
        bind_user(toolset, superuser).workflow_cancel(workflow_state_id=999_999)
